...
describe.skip('Functors', () => {
  ...
  test("Write a function that will getPost then toUpperCase the post's title.", () => {
    ...
    const getPost = (id) =>
      Async((_, res) =>
        setTimeout(() => res({ id, title: 'Love them futures' }), 300),
      );
    ...
    getPostThenUpper(1).fork(console.log, (title) =>
  ...
  test('Write a function that uses checkActive() and showWelcome() to grant access or return the error.', () => {
    ...
    const checkActive = (user) =>
      user.active ? Right(user) : Left('Your account is not active');
    // TODO: eitherWelcome :: User -> Either String String
    const eitherWelcome = identity;
    expect(
      eitherWelcome({ name: 'Flavio', active: true }).equals(
        Right('Welcome Flavio'),
      ),
    ).toBeTruthy();
    expect(
      eitherWelcome({ name: 'Yannick', active: false }).equals(
